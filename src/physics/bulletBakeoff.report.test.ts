...
const reportIt = process.env.POG_BULLET_BAKEOFF ? it : it.skip
...
const seeds = Array.from({ length: 8 }, (_, index) => 'bullet-' + index)
...
reportIt(
  'compares Rapier and Bullet on identical real-scale POG shots',
  async () => {
    ...
    for (const scenario of scenarios) {
      for (const slammerMassKg of slammerMasses) {
        ...
        for (const seed of seeds) {
          ...
          bullet.push(await simulateBulletShot(input))
        }
        ...
        report[key] = {
          ...
          bullet: summarize(bullet),
        }
      }
    }

    console.log(
      'POG_BULLET_BAKEOFF=' + JSON.stringify(report, null, 2),
    )
